...
no, not all the file will pushed to the remote, just the code.py will be pushed.
because the .gitignore file contain all files has "txt" extention.
...
not correct.
because code.py still untracted file, we should put it in the staging area then commit it.
